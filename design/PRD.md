...
- **配置项**：
  - AI 模型凭据：列表展示用户绑定的模型（如 DeepSeek、GPT-5、Moonshot 等），支持新增/编辑/停用；工作台仅展示启用且验证通过的模型（现已落地——使用抽屉表单录入凭据）。
  - API Key 管理：每个模型凭据单独存储 Key / Endpoint / 其他配置，可触发“测试连通性”，通过后标记为启用。前端在抽屉底部提供“测试连通性”按钮，成功/失败均以 toast + 字段校验反馈表现。
  - 同步：开关、默认同步路径、最近同步时间。（UI 已存在占位，后续将联通同步接口）
  - 数据路径：显示当前 SQLite 文件路径，可修改（需重启后生效）。
- **验收标准**：修改后保存成功提示，错误需高亮字段并给出说明。当前实现中，提交表单时会针对空值/格式错误高亮字段并显示后端返回的错误文案。
...
- 操作日志：`/api/changelog` + 本地日志列表已提供，记录生成、同步、删除等关键行为，保留 30 天。UI 端在“帮助与日志”页面展示操作日志表格，支持过滤与导出。
