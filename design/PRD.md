...
- 提供一款桌面端 Prompt 生产力工具，支持本地和云端双存储，帮助用户在 5 分钟内从需求到可用 Prompt。
- 支持围绕面试等高频场景的模板化生成与关键词治理，构建中心化的 Prompt 资产库。
- 打好扩展基础，未来可引入 GPT-5、DeepSeek 等模型，实现多模型选择与协同。
...
- **稳定**：本地 SQLite 提供离线访问，云端 MySQL 保证多端同步，数据不丢失。
- **扩展**：支持多模型（DeepSeek、GPT-5 等）与自定义模板，满足不同领域。
...
2. 主题检索（本地 + 云端）与关键词分类（正面/负面）。
3. Prompt 自动生成、编辑、保存、删除功能。
4. 本地 SQLite 存储；手动触发的云端同步（含冲突提示）。
5. AI 模型接入 DeepSeek & GPT-5（开启即用，默认 DeepSeek，可切换）。
6. 设置页：API Key 管理、模型选择、同步开关、本地数据库路径。
...
- 用户具备可用的 AI API Key（DeepSeek/GPT-5）；若无需登录则提供公用 Key 限额。
- 桌面端需安装 Go 运行环境或打包后端二进制。
- 云端 MySQL/对象存储由后端团队统一提供。
- 法务与安全：本地数据加密及云端符合数据合规要求。
...
| US-03 | Prompt 工程师 | 管理本地 + 云端 Prompt | 本地列表支持搜索/过滤；同步后云端显示更新提示 |
| US-04 | 所有用户 | 更换模型（DeepSeek / GPT-5） | 设置页切换模型后，下次生成默认使用最新模型，并在结果中展示来源标识 |
...
- **功能点**：主题搜索、历史记录、快速入口。
- **描述**：用户输入主题后，系统展示匹配的本地/云端模板及关键词，检索可以考虑使用Elasticsearch，采用每个 prompts 的 topic 和 权重大的 keyword 进行检索，若无，则提示“未找到，尝试生成”。
- **验收标准**：
  1. 搜索响应时间 ≤ 1 秒（命中本地数据）。
...
- **流程**：
  ...
  2. 并行调用 AI 模型生成补充关键词（默认 DeepSeek，可选 GPT-5），并与解析结果合并去重复；该调用在用户点击“向 AI 补充关键词”或解析结果明显不足时触发，返回内容标记来源=`api`。
...
- **验收标准**：
  ...
  - 提供“再次生成”按钮，保留用户调整的关键词；需对 60 秒内的调用次数设置限流（默认 ≤3 次），超限时提示“请稍后再试”。
...
- **需求补充**：
  - 数据需加密存储（SQLite 加密或应用层加密）。
  - 提供备份导出功能（JSON/CSV）。

### 4.6 云端同步

- **模式**：用户登录后可选择“立即同步”或“自动同步”（默认关闭）。
- **流程**：
  1. 对比本地与云端更新时间戳。
  2. 冲突时提示用户选择“以本地为准 / 以云端为准 / 保留两份”。
- **验收标准**：
  - 同步结果以 toast 告知（成功 / 失败 / 冲突）。
  - 失败需可重试并在日志页查看原因。
  - 仅 `published` 状态的 Prompt 参与云端同步；`draft` 仅保留本地，可在“我的 Prompt”中手动发布。

### 4.7 设置页

- **配置项**：
  - AI 模型凭据：列表展示用户绑定的模型（如 DeepSeek、GPT-5、Moonshot 等），支持新增/编辑/停用；工作台仅展示启用且验证通过的模型。
  - API Key 管理：每个模型凭据单独存储 Key / Endpoint / 其他配置，可触发“测试连通性”，通过后标记为启用。
  - 同步：开关、默认同步路径、最近同步时间。
  - 数据路径：显示当前 SQLite 文件路径，可修改（需重启后生效）。
- **验收标准**：修改后保存成功提示，错误需高亮字段并给出说明。
  - API Key 保存需通过操作系统安全组件（Windows DPAPI / macOS Keychain）或项目内置 AES-256 + 用户主密码加密，解密仅限即时调用；禁止明文写入日志、网络报文或同步包。
- **接口**：
...
- **输入与校验**：在设置页提供独立抽屉或弹窗录入模型凭据（模型标识、展示名、API Key、Base URL、附加参数），提交前进行基本格式校验与“测试连通性”按钮，调用后端短请求验证有效性并返回具体错误。
...
- **同步策略**：API Key 不同步至云端；如需在多设备使用，用户需在每台设备单独录入，可导出加密备份。
- **日志与审计**：所有相关日志只记录操作结果（成功/失败）与错误码，不包含 Key 内容；操作写入审计表以便排查。

### 4.8 日志与反馈

- 操作日志：记录生成、同步、删除等关键行为，保留 30 天。
- 系统反馈：统一 toast + 日志面板；严重错误可导出日志反馈给支持团队。
...
| settings | json / text | 用户自定义设置（模型、同步选项等），SQLite 使用 JSON 字段，MySQL 5.7 以 TEXT 存储 JSON 字符串 |
...
- `prompts`：对 `user_id, topic, status` 建复合索引用于列表过滤；为 `updated_at` 创建索引用于同步比对；`user_id + topic + created_at` 设唯一约束避免重复生成。
...
- `user_model_credentials`：对 `user_id + model_key` 设唯一约束，保证同一模型配置仅出现一次；为 `status`、`updated_at` 建索引便于筛选启用模型与同步校验任务。
...
- **全局框架**：
  - 顶部固定跨页搜索条（支持主题、关键词模糊搜索）+ 全局操作区（“新建 Prompt”主按钮、最近活动、同步状态指示）。
  - 左侧导航栏：`Dashboard`、`我的 Prompt`、`模板中心（预留）`、`设置`、`帮助与日志`；支持折叠。
- **主要页面 / 区域**：

| 区域 | 入口 | 主要模块 | 说明 |
| --- | --- | --- | --- |
| Dashboard 仪表盘 | 默认首页 | 快速搜索、最近访问 Prompt、草稿提醒、模型额度状态 | 作为登陆后的主视图，集中展示关键提醒和快捷入口（“继续编辑草稿”、“发布待办”）。|
| Prompt 工作台 | 顶部“新建 Prompt”或“编辑”入口 | 左侧关键词面板（正/负面 Tag + 自定义输入）、右侧 Prompt 预览与元信息面板 | 关键词与 Prompt 强关联，使用双栏结构；顶部步骤条提示“关键词 → Prompt → 发布”。|
| 我的 Prompt | 左侧导航 | 列表、筛选、排序、批量操作、状态筛选（draft/published/archived） | 支持标签筛选、批量发布/归档；行内提供快捷查看和进入工作台。|
| 设置中心 | 左侧导航 | 账号信息、模型配置、同步策略、本地路径 | 同一页签内使用分组卡片，提供“连接测试”“重置路径”等操作。|
| 帮助与日志 | 左侧导航 | FAQ、错误日志、导出按钮、联系客服入口 | 整合系统日志查看与反馈渠道。|

### 6.2 关键 UX 原则

- 主要操作最多 3 步完成。
- 强反馈：生成 / 同步需展示 loading、进度。
...
3. 点击“保存草稿”留在本地，状态为 `draft`，列表页面会出现草稿提醒；点击“发布”触发校验、调用后端生成最终 Prompt，并写入云端，状态切换为 `published`。
4. 发布后的 Prompt 可在“我的 Prompt”中再次编辑（生成新的草稿版本）或归档；归档后默认保存在本地和云端，不参与同步列表。
...
- **后端（Go）**：
  ...
  - 提供健康检查 `/api/status`、关键词生成 `/api/generate_keywords`、Prompt 生成 `/api/generate_prompt`、模型凭据管理 `/api/models`、同步相关 API。
  - 集成 DeepSeek、GPT-5 模型 SDK，支持并发请求、超时重试与熔断（go-resilience）。
- **数据库**：
  - 本地 SQLite（加密），通过 GORM 访问，统一迁移脚本管理。
  - 云端 MySQL，结构与本地一致，额外 `user_id` 约束与审计字段。
- **同步方案**：
  - 差异化同步（比较 `updated_at`），冲突由后端返回冲突包。
  - 异常日志写入本地文件，并可自助上传至支持端。
...
- **测试与质量**：每个模块至少配备 1 条 Happy Path + 1 条边界单测；关键流程（生成、同步）补充集成测试，前端使用 Playwright 进行冒烟测试。
...
- **商业指标**：高级模型 / GPT-5 使用率，潜在订阅转化（预留）。
- **运营动作**：上线期通过引导模板包（热门面试岗位）提升活跃；提供内置示例库。
- **埋点需求**：页面曝光、按钮点击、生成结果状态、同步结果等。
...
| GPT-5 接口不稳定 | 影响生成体验 | 默认使用 DeepSeek，GPT-5 作为可选项并提示状态 |
| 云端同步冲突多 | 数据丢失风险 | 引入冲突提示 + 日志记录，迭代自动合并策略 |
...
> 说明：本地默认使用 SQLite，云端使用 MySQL。以下给出双版本 DDL，便于直接复制执行。
