...
- 法务与安全：本地数据需加密存储，并在未来扩展云端能力前完成合规评估。
...
- **验收标准**：
  ...
  - 默认保存为 `draft` 状态，自动本地草稿保存（每 5 秒或关键操作后触发），用户显式点击“发布”后才变为 `published` 并记录新的版本快照。
...
- **模式**：提供一键导出（JSON/CSV）与导入能力，保障本地数据安全。
- **流程**：
  1. 用户在“我的 Prompt”或设置页点击“导出”即生成本地文件。
...
- **配置项**：
  ...
  - 数据导出：提供 JSON/CSV 导出入口，并记录最近一次导出时间。（UI 已存在占位，后续将联通导入能力）
