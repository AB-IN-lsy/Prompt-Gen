...
- **Prompt 工作台（新版）**：新增“自然语言需求解析 → AI 补词 → 模型生成 → 草稿/发布”一站式流程；支持实时解析置信度、模型可用性联动、手动/自动关键词去重与来源标记。
- **关键词权重与拖拽**：正、负向关键词支持 0~5 的相关度权重，点击 ± 可调整，并可拖拽跨列移动；前端会调用 `/api/prompts/keywords/sync` 将排序与权重同步到 Redis 工作区，确保生成/保存对齐。
...
- **应用外壳**：`AppShell` 搭建统一导航、顶部工具栏、账户信息与退出功能，展示后端返回的用户头像并在退出时提示成功。导航栏已分为“普通入口”和“管理后台”两段：普通用户只会看到仪表盘、Prompt 工作台、帮助等菜单；管理员登录后，在侧边栏下半段会出现“管理后台”分隔标题，包含 IP 防护与更新日志管理页面入口。
