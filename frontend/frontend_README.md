...
- **优质 Prompt 库**：新增 “优质 Prompt 库” 页面，支持按关键词、状态筛选公共 Prompt，并一键导入到个人库（离线模式保留浏览与下载能力，投稿在在线模式开放）；普通用户在筛选“待审核 / 已驳回”时只会看到自己的投稿记录。
...
- **公共库审核面板**：管理后台追加“公共库审核”菜单，提供分页搜索、快速审批与驳回原因记录，并新增“删除公共 Prompt”按钮，支持在审核前后移除不合规条目；对应 `/admin/public-prompts` 页面完成投稿审核流。
